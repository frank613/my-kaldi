...
if [ $stage -le 4 ]; then
	if [ ! -d exp/align_gop_mono_for_nnet ];then
		steps/align_si_gop_nnet2.sh --cmd "$train_cmd" \
                    data/data_cmu_gop/test $lang_path $si_model_path exp/align_gop_mono_for_nnet
	fi
fi

# Denominator
if [ $stage -le 6 ]; then
	steps/gop-nnet2-likeRatio.sh  $nn_model_path exp/align_gop_mono_for_nnet data/data_cmu_gop/test exp/gop_nnet2_mono
fi 
