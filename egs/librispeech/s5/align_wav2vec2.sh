...
if [ $stage -le 13 ]; then
  steps/train_mono.sh  --boost-silence 1.25 --nj 20 --cmd "$train_cmd"  \
                      data_30/train_clean_100 data/lang_nosp  exp/mono_all_data_30

fi
echo "done with trainng"


if [ $stage -le 16 ]; then
  if [ ! -d exp/align_train100_30 ];then
        steps/align_si_gop_ctm.sh --cmd "$train_cmd" \
                    --nj 20 data_30/train_clean_100 data/lang_nosp exp/mono_all_data_30 exp/align_train100_30
  fi	
fi
