...
aliDir=exp/mono_ali_v3/$fname
gopDir=exp/gop_replace_all_v3/$fname
...
if [ $stage -le 4 ]; then
	#get the aligment score given target phoneme (using the middle HMM state) 
  local/compute_AMs_given_phoneme.sh --nj 2 --cmd "$train_cmd" \
	  data/gop_combined_test/extracted_segments/$targetP/$toP exp/mono_all_data $aliDir
fi
