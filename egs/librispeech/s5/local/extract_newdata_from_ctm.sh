...
./local/replace_stress.sh text.orig $targetP $toP | awk -F' ' '{print($1,"P_"$2)}' | sort > $workdir/text || exit 1
...
cat $workdir/words.txt | cut -d' ' -f1 | sed 's/P_//g' > $workdir/phone_list.tmp
...
cat $workdir/phones.txt | awk -F' ' '{print "P_"$1,"1.0",$1}' | tail -n +2 > $workdir/lexiconp.txt
