...
if [ $stage -le 0 ]; then
  ...
  if [ ! -d exp/align_gop_mono_for_nnet ];then
  	steps/align_si_gop_nnet2.sh --cmd "$train_cmd" \
                    data/gop_combined data/lang_nosp  exp/mono_all_data exp/align_gop_mono_for_nnet
  fi
fi
...
if [ $stage -le 1 ]; then
  #GOP score using nnet2 for likeRatio 
  	...
  	steps/gop-nnet2-likeRatio.sh  exp/nnet5a_clean_100_gpu exp/align_gop_mono_for_nnet data/gop_combined exp/gop_nnet2_mono
  #fi
