...
selected=${DATA_ROOT}/selected-accented
...
./local/voxforge_format_data_gop_accented.sh || exit 1
...
mfccdir=${DATA_ROOT}/mfcc-accented
#for x in train test; do
steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj \
  data_accented/test exp/make_mfcc_accented/test $mfccdir || exit 1;
steps/compute_cmvn_stats.sh data_accented/test exp/make_mfcc_accented/test $mfccdir || exit 1;
fi
...
if [ $stage -le 4 ]; then
        if [ ! -d exp/align_gop_mono_for_nnet ];then
                steps/align_si_gop_nnet2.sh --cmd "$train_cmd" \
                    data_accented/test $lang_path $si_model_path exp/align_gop_mono_for_nnet
        fi
fi

if [ $stage -le 6 ]; then
        steps/gop-nnet2-likeRatio.sh  $nn_model_path exp/align_gop_mono_for_nnet data_accented/test exp/gop_nnet2_tri
fi
