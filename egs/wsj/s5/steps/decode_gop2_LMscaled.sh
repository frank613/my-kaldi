...
beam=16
lattice_beam=10
acwt=1 # note: only really affects pruning (scoring is on lattices).
...
if [ $# != 5 ]; then
   echo "Usage: steps/decode_gop2.sh [options] <ali-dir> <graph-dir> <data-dir> <decode-dir> <lang-dir>"
...
if [ $stage -le 0 ]; then
  ...
  ##larger acoustic scale,  no partial output compared to v1. Disable all dertminization for state-level lattice
  $cmd JOB=1:$nj $dir/log/decode.JOB.log \
    gmm-latgen-faster  --beam=$beam --lattice-beam=$lattice_beam \
    --acoustic-scale=$acwt --allow-partial=false --determinize-lattice=false $decode_extra_opts \
...
if [ $stage -le 1 ]; then
  #CTM file generation 
  $cmd JOB=1:$nj $dir/log/1best-ctm.JOB.log \
  lattice-1best "ark:gunzip -c $dir/lat.JOB.gz|" ark:- \| lattice-align-phones $model ark:- ark:- \| nbest-to-ctm ark:- "$dir/JOB.ctm.ark"  || exit 1;
  cat $dir/*.ctm.ark | utils/int2sym.pl -f 5- $graphdir/words.txt >> "$dir/all.denom.ctm" || exit 1;
