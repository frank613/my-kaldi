...
if [ $stage -le 0 ]; then
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd JOB=1:$nj $dir/log/get_word_align.JOB.log \
      ...
      utils/int2sym.pl -f 3 $lang/words.txt '>' $ali_dir/word_aligned.JOB.lat || exit 1
  else
      echo "not impletemented for the second case, refer to the original script get_train_ctm.sh "	  
      exit 1;
  fi
fi
