...
N=1  # change N and P for non-trigram systems.
#P=1
P=0
...
if [ $# -ne 4 ]; then
  echo "Usage: $0  [options] <lang-dir> <alignment-dir> <model-dir> <working-dir>"
...
#link the model to the working directory
if [ -d $dir ]; then
  rm -rf $dir
fi
mkdir -p $dir
